...
.controller(
		...
		[
		 ...
		 '$rootScope',
		 'injectedData',
		 function($scope, feeService, masterdataService,
				 admissionService, $modal, $state, $rootScope,
				 injectedData) {
			 ...
			 $scope.init = function() {
				 ...
				 console
				 .log('getting masterdata for Fee module in init block');
			 ...
			 $scope.updateAmenityCharges = function(temp) {
				 ...
				 angular
				 .forEach(
						 $scope.serverModelData.dropdownMasterData.AMENITIES,
						 function(amenity) {
			 ...
			 $scope.getMaxListLengthArray = function() {
				 var size = 0;
				 if ($scope.feeTransactionAdmissionBean
						 && $scope.feeTransactionAdmissionBean.feeTransactionDebit
						 && $scope.feeTransactionAdmissionBean.feeTransactionCredit) {
					 size = Math
					 .max(
							 $scope.feeTransactionAdmissionBean.feeTransactionDebit.length,
							 $scope.feeTransactionAdmissionBean.feeTransactionCredit.length);
				 }
				 if (size > 0) {
			 ...
			 $scope.totalBaseFee = function() {
				 var sum = 0;
				 if ($scope.admissionDetailManagement
						 && $scope.admissionDetailManagement.applicableFeeDetails) {
					 angular
					 .forEach(
							 $scope.admissionDetailManagement.applicableFeeDetails,
			 ...
			 $scope.addNewDiscount = function() {
				 ...
				 $scope.admissionDetailManagement.stagingFee
				 .push(discount);
			 }
			 ...
			 $scope.directViewFeeDeposite = function(
					 currentFileNo) {
			 ...
			 $scope.getFeeTransactionAndBasicInfoDetail = function(
					 fileNo) {
				 console
				 .log('FeeTransactionAndBasicInfoDetail called in controller');
				 feeService
				 .getFeeTransactionAndBasicInfoDetail(
						 fileNo)
						 .then(
								 function(response) {
									 console
			 ...
			 $scope.getPendingFee = function() {
				 ...
				 feeService
				 .getPendingFee($scope.currentFetchLimit)
				 .then(
						 function(response) {
							 console
							 .log('pending fee data received in controller : ');
							 console.log(response);
							 if (response != null
									 && response.data != null
									 && response.data.responseBody != null) {
			 ...
			 $scope.depositeFee = function() {
				 ...
				 console
				 .log('fee deposite called in controller');

				 feeService
				 .depositeFee($scope.newTransaction)
				 .then(
						 function(response) {
							 console
							 .log('Data received from service : ');
							 ...
							 if (response != null
								 ...
								 $scope.newTransaction = {};
								 $scope
								 .getFeeTransactionAndBasicInfoDetail($scope.feeTransactionAdmissionBean.basicInfo.fileNo);
							 } else {
								 console
								 .log(response.data.error);
								 alert(response.data.error);
			 ...
			 $scope.getAdmissionDetailManagement = function(
				 ...
				 .then(
						 function(response) {
							 console
							 .log('Admission detail for management received from service : ');
							 console.log(response);
							 if (response != null
									 && response.data != null
									 && response.data.responseBody != null) {
			 ...
			 $scope.updateAdmissionDetailManagement = function() {
				 feeService
				 .updateAdmissionDetailManagement(
						 $scope.admissionDetailManagement)
						 .then(
								 function(response) {
									 console
									 .log('update Admission detail for management received from service in controller : ');
									 console.log(response);
									 if (response != null
											 && response.data != null
											 && response.data.responseBody != null) {
